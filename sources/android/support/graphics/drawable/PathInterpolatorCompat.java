...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f8mX = new float[numPoints];
        this.f9mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
            this.f8mX[i] = position[0];
            this.f9mY[i] = position[1];
        }
        if (((double) Math.abs(this.f8mX[0])) > 1.0E-5d || ((double) Math.abs(this.f9mY[0])) > 1.0E-5d || ((double) Math.abs(this.f8mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f9mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f8mX[0] + "," + this.f9mY[0] + " end:" + this.f8mX[numPoints - 1] + "," + this.f9mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        int i2 = 0;
        ...
        while (i2 < numPoints) {
            ...
            float x = this.f8mX[componentIndex];
            if (x < prevX) {
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f8mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f8mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f8mX[endIndex] - this.f8mX[startIndex];
        if (xRange == 0.0f) {
            return this.f9mY[startIndex];
        }
        float fraction = (t - this.f8mX[startIndex]) / xRange;
        float startY = this.f9mY[startIndex];
        return ((this.f9mY[endIndex] - startY) * fraction) + startY;
    }
}
