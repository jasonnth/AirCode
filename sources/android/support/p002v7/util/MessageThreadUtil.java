...
import android.support.p002v7.util.ThreadUtil.BackgroundCallback;
import android.support.p002v7.util.ThreadUtil.MainThreadCallback;
import android.support.p002v7.util.TileList.Tile;
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new Runnable() {
                public void run() {
                    SyncQueueItem msg = C02171.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C02192.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C02192.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C02192.this.mQueue.removeMessages(2);
                                    C02192.this.mQueue.removeMessages(3);
                            ...
                            C02192.this.mBackgroundRunning.set(false);
                            return;
                        }
                    }
                }
