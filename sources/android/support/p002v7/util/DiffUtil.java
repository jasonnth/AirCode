...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f27x - o2.f27x;
            return cmpX == 0 ? o1.f28y - o2.f28y : cmpX;
        }
    };
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f27x != 0 || firstSnake.f28y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f27x + snake.size;
                int endY = snake.f28y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f27x + j;
                    int newItemPos = snake.f28y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f27x + snakeSize;
                int endY = snake.f28y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f27x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f27x + i, 1, this.mCallback.getChangePayload(snake.f27x + i, snake.f28y + i));
                    }
                }
                posOld = snake.f27x;
                posNew = snake.f28y;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!arrayList.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f27x += range.oldListStart;
                snake.f28y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f27x;
                    left.newListEnd = snake.f28y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f27x - 1;
                    left.newListEnd = snake.f28y;
                } else {
                    left.oldListEnd = snake.f27x;
                    left.newListEnd = snake.f28y - 1;
                }
                arrayList.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f27x + snake.size;
                    right.newListStart = snake.f28y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f27x + snake.size + 1;
                    right.newListStart = snake.f28y + snake.size;
                } else {
                    right.oldListStart = snake.f27x + snake.size;
                    right.newListStart = snake.f28y + snake.size + 1;
                }
                arrayList.add(right);
            } else {
        ...
        for (int d = 0; d <= dLimit; d++) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f27x = backward[kOffset + k];
                    outSnake.f28y = outSnake.f27x - k;
            ...
            while (k2 <= d) {
                ...
                if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f27x = backward[kOffset + backwardK];
                    outSnake2.f28y = outSnake2.f27x - backwardK;
