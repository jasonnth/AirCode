...
public class ButtonBarLayout extends LinearLayout {
    ...
    public ButtonBarLayout(Context context, AttributeSet attrs) {
        ...
        super(context, attrs);
        if (ConfigurationHelper.getScreenHeightDp(getResources()) >= ALLOW_STACKING_MIN_HEIGHT_DP) {
            allowStackingDefault = true;
        }
        TypedArray ta = context.obtainStyledAttributes(attrs, C0199R.styleable.ButtonBarLayout);
        this.mAllowStacking = ta.getBoolean(C0199R.styleable.ButtonBarLayout_allowStacking, allowStackingDefault);
        ta.recycle();
    }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mAllowStacking && !isStacked()) {
            if (VERSION.SDK_INT >= 11) {
                stack = (ViewCompat.getMeasuredWidthAndState(this) & AirMapInterface.CIRCLE_BORDER_COLOR) == 16777216;
            } else {
    ...
    private void setStacked(boolean stacked) {
        ...
        View spacer = findViewById(C0199R.C0201id.spacer);
        if (spacer != null) {
