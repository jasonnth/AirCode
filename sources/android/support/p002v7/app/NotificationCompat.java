...
import android.support.p000v4.app.NotificationCompat.MessagingStyle.C0108Message;
...
public class NotificationCompat extends android.support.p000v4.app.NotificationCompat {
    ...
    public static class Builder extends android.support.p000v4.app.NotificationCompat.Builder {
        ...
        public CharSequence resolveText() {
            if (this.mStyle instanceof MessagingStyle) {
                ...
                C0108Message m = NotificationCompat.findLatestIncomingMessage(style);
    ...
    /* access modifiers changed from: private */
    public static C0108Message findLatestIncomingMessage(MessagingStyle style) {
        List<C0108Message> messages = style.getMessages();
        for (int i = messages.size() - 1; i >= 0; i--) {
            C0108Message m = (C0108Message) messages.get(i);
            if (!TextUtils.isEmpty(m.getSender())) {
                return m;
            }
        }
        if (!messages.isEmpty()) {
            return (C0108Message) messages.get(messages.size() - 1);
        }
        return null;
    }

    /* access modifiers changed from: private */
    public static CharSequence makeMessageLine(android.support.p000v4.app.NotificationCompat.Builder b, MessagingStyle style, C0108Message m) {
        BidiFormatter bidi = BidiFormatter.getInstance();
        SpannableStringBuilder sb = new SpannableStringBuilder();
        boolean afterLollipop = VERSION.SDK_INT >= 21;
        int color = (afterLollipop || VERSION.SDK_INT <= 10) ? AirMapInterface.CIRCLE_BORDER_COLOR : -1;
    ...
    private static boolean hasMessagesWithoutSender(List<C0108Message> messages) {
        for (int i = messages.size() - 1; i >= 0; i--) {
            if (((C0108Message) messages.get(i)).getSender() == null) {
                return true;
            }
        }
        return false;
    ...
    private static void setBackgroundColor(Context context, RemoteViews views, int color) {
        if (color == 0) {
            color = context.getResources().getColor(C0199R.color.notification_material_background_media_default_color);
        }
        views.setInt(C0199R.C0201id.status_bar_latest_event_content, "setBackgroundColor", color);
    }
