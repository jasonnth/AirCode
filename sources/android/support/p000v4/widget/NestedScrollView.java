...
public class NestedScrollView extends FrameLayout implements NestedScrollingChild, NestedScrollingParent, ScrollingView {
        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case 20:
                        if (event.isAltPressed()) {
                            handled = fullScroll(ISO781611.BIOMETRIC_SUBTYPE_TAG);
                            break;
                        } else {
                            handled = arrowScroll(ISO781611.BIOMETRIC_SUBTYPE_TAG);
                            break;
                        }
                    case 62:
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, ISO781611.BIOMETRIC_SUBTYPE_TAG);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(ISO781611.BIOMETRIC_SUBTYPE_TAG)) {
                return false;
            }
            return true;
        }
    ...
    private void onSecondaryPointerUp(MotionEvent ev) {
        int pointerIndex = (ev.getAction() & ExploreBaseRangeSeekBar.ACTION_POINTER_INDEX_MASK) >> 8;
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = ISO781611.BIOMETRIC_SUBTYPE_TAG;
        } else if (direction == 1) {
            direction = 33;
        }
