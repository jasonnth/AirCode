...
public final class Response<T> {
    ...
    public static <T> Response<T> success(T body2) {
        return success(body2, new Builder().code(200).message(OfficialIdStatusResponse.f1090OK).protocol(Protocol.HTTP_1_1).request(new Request.Builder().url("http://localhost/").build()).build());
    }

    public static <T> Response<T> success(T body2, Headers headers) {
        if (headers != null) {
            return success(body2, new Builder().code(200).message(OfficialIdStatusResponse.f1090OK).protocol(Protocol.HTTP_1_1).headers(headers).request(new Request.Builder().url("http://localhost/").build()).build());
        }
    ...
    public static <T> Response<T> error(int code, ResponseBody body2) {
        if (code >= 400) {
            return error(body2, new Builder().code(code).protocol(Protocol.HTTP_1_1).request(new Request.Builder().url("http://localhost/").build()).build());
        }
