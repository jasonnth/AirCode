...
public class GF2mMatrix extends Matrix {
    ...
    public byte[] getEncoded() {
        ...
        bf[0] = (byte) (this.numRows & 255);
        bf[1] = (byte) ((this.numRows >>> 8) & 255);
        bf[2] = (byte) ((this.numRows >>> 16) & 255);
        bf[3] = (byte) ((this.numRows >>> 24) & 255);
        int count2 = 4;
        for (int i = 0; i < this.numRows; i++) {
            int j = 0;
            while (j < this.numColumns) {
    ...
    public Matrix computeInverse() {
        ...
        int[][] tmpMatrix = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.numRows, this.numRows});
        for (int i = this.numRows - 1; i >= 0; i--) {
            tmpMatrix[i] = IntUtils.clone(this.matrix[i]);
        }
        int[][] invMatrix = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.numRows, this.numRows});
