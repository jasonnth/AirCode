...
public class GF2Matrix extends Matrix {
    ...
    private void assignRandomRegularMatrix(int n, SecureRandom sr) {
        ...
        this.length = (n + 31) >>> 5;
        this.matrix = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.numRows, this.length});
        GF2Matrix rm = (GF2Matrix) new GF2Matrix(n, Matrix.MATRIX_TYPE_RANDOM_LT, sr).rightMultiply((Matrix) new GF2Matrix(n, Matrix.MATRIX_TYPE_RANDOM_UT, sr));
    ...
    public static GF2Matrix[] createRandomRegularMatrixAndItsInverse(int n, SecureRandom sr) {
        ...
        GF2Matrix gF2Matrix = new GF2Matrix(n, Matrix.MATRIX_TYPE_RANDOM_UT, sr);
        GF2Matrix rm = (GF2Matrix) lm.rightMultiply((Matrix) gF2Matrix);
        Permutation p = new Permutation(n, sr);
        int[] pVec = p.getVector();
        int[][] matrix2 = (int[][]) Array.newInstance(Integer.TYPE, new int[]{n, length2});
        for (int i = 0; i < n; i++) {
            System.arraycopy(rm.matrix[pVec[i]], 0, matrix2[i], 0, length2);
        }
        GF2Matrix gF2Matrix2 = new GF2Matrix(n, matrix2);
        result[0] = gF2Matrix2;
        ...
        for (int i3 = n - 1; i3 >= 0; i3--) {
            ...
            for (int j2 = i3 - 1; j2 >= 0; j2--) {
                if ((gF2Matrix.matrix[j2][q2] & r2) != 0) {
    ...
    public byte[] getEncoded() {
        ...
        while (i < this.numRows) {
            ...
            while (j2 < r) {
                int count3 = count2 + 1;
                enc[count2] = (byte) ((this.matrix[i][q] >>> j2) & 255);
                j2 += 8;
                count2 = count3;
            }
            i++;
    ...
    public GF2Matrix getLeftSubMatrix() {
        ...
        int[][] result = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.numRows, length2});
    ...
    public Matrix computeTranspose() {
        int[][] result = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.numColumns, (this.numRows + 31) >>> 5});
        for (int i = 0; i < this.numRows; i++) {
    ...
    public Matrix computeInverse() {
        ...
        int[][] tmpMatrix = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.numRows, this.length});
        for (int i = this.numRows - 1; i >= 0; i--) {
            tmpMatrix[i] = IntUtils.clone(this.matrix[i]);
        }
        int[][] invMatrix = (int[][]) Array.newInstance(Integer.TYPE, new int[]{this.numRows, this.length});
