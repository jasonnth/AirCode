...
public class GMSSPrivateKeyParameters extends GMSSKeyParameters {
    ...
    public GMSSPrivateKeyParameters(int[] index2, byte[][] currentSeeds2, byte[][] nextNextSeeds2, byte[][][] currentAuthPaths2, byte[][][] nextAuthPaths2, byte[][][] keep2, Treehash[][] currentTreehash2, Treehash[][] nextTreehash2, Vector[] currentStack2, Vector[] nextStack2, Vector[][] currentRetain2, Vector[][] nextRetain2, GMSSLeaf[] nextNextLeaf2, GMSSLeaf[] upperLeaf2, GMSSLeaf[] upperTreehashLeaf2, int[] minTreehash2, byte[][] nextRoot2, GMSSRootCalc[] nextNextRoot2, byte[][] currentRootSig2, GMSSRootSig[] nextRootSig2, GMSSParameters gmssParameterset, GMSSDigestProvider digestProvider2) {
        ...
        this.f7084K = gmssParameterset.getK();
        ...
        this.currentAuthPaths = currentAuthPaths2;
        ...
        if (nextNextRoot2 == null) {
            ...
            for (int i5 = 0; i5 < this.numLayer - 1; i5++) {
                this.nextNextRoot[i5] = new GMSSRootCalc(this.heightOfTrees[i5 + 1], this.f7084K[i5 + 1], this.digestProvider);
            }
        } else {
    ...
    private void nextTree(int layer) {
        if (layer > 0) {
            ...
            if (!lastTree) {
                ...
                for (int i2 = 0; i2 < this.heightOfTrees[layer] - this.f7084K[layer]; i2++) {
                ...
                for (int i4 = 0; i4 < this.f7084K[layer] - 1; i4++) {
                ...
                byte[] OTSseed = this.gmssRandom.nextSeed(dummy);
                byte[] OTSseed2 = this.gmssRandom.nextSeed(dummy);
    ...
    private void updateKey(int layer) {
        ...
        if (layer > 0) {
            ...
            this.upperLeaf[layer - 1] = this.upperLeaf[layer - 1].nextLeaf();
            int t = (int) Math.floor(((double) (getNumLeafs(layer) * 2)) / ((double) (this.heightOfTrees[layer - 1] - this.f7084K[layer - 1])));
    ...
    private int getMinTreehashIndex(int layer) {
        int minTreehash2 = -1;
        for (int h = 0; h < this.heightOfTrees[layer] - this.f7084K[layer]; h++) {
    ...
    private void computeAuthPaths(int layer) {
        ...
        if (Tau == 0) {
            if (layer == this.numLayer - 1) {
                WinternitzOTSignature winternitzOTSignature = new WinternitzOTSignature(OTSseed, this.digestProvider.get(), this.otsIndex[layer]);
                help = winternitzOTSignature.getPublicKey();
            } else {
                byte[] dummy = new byte[this.mdLength];
            ...
            for (int i2 = 0; i2 < Tau; i2++) {
                ...
                if (i2 < H - K && Phi + ((1 << i2) * 3) < this.numLeafs[layer]) {
            ...
            for (int tmp = 1; tmp <= (H - K) / 2; tmp++) {
                ...
                if (minTreehash2 >= 0) {
                    try {
                        ...
                        WinternitzOTSignature winternitzOTSignature2 = new WinternitzOTSignature(this.gmssRandom.nextSeed(seed), this.digestProvider.get(), this.otsIndex[layer]);
                        byte[] leaf = winternitzOTSignature2.getPublicKey();
                        this.currentTreehash[layer][minTreehash2].update(this.gmssRandom, leaf);
                    } catch (Exception e) {
                        System.out.println(e);
                    }
                }
