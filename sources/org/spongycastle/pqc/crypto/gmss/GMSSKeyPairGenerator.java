...
public class GMSSKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
    ...
    private AsymmetricCipherKeyPair genKeyPair() {
        ...
        for (int i = 0; i < this.numLayer; i++) {
            currentAuthPaths[i] = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{this.heightOfTrees[i], this.mdLength});
            currentTreehash[i] = new Treehash[(this.heightOfTrees[i] - this.f7079K[i])];
            if (i > 0) {
                nextAuthPaths[i - 1] = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{this.heightOfTrees[i], this.mdLength});
                nextTreehash[i - 1] = new Treehash[(this.heightOfTrees[i] - this.f7079K[i])];
            }
        ...
        byte[][] currentRoots = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{this.numLayer, this.mdLength});
        byte[][] nextRoots = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{this.numLayer - 1, this.mdLength});
        byte[][] seeds = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{this.numLayer, this.mdLength});
        ...
        for (int h = this.numLayer - 1; h >= 0; h--) {
            GMSSRootCalc tree = new GMSSRootCalc(this.heightOfTrees[h], this.f7079K[h], this.digestProvider);
            try {
        ...
        GMSSPublicKeyParameters gMSSPublicKeyParameters = new GMSSPublicKeyParameters(currentRoots[0], this.gmssPS);
        return new AsymmetricCipherKeyPair((AsymmetricKeyParameter) gMSSPublicKeyParameters, (AsymmetricKeyParameter) new GMSSPrivateKeyParameters(this.currentSeeds, this.nextNextSeeds, currentAuthPaths, nextAuthPaths, currentTreehash, nextTreehash, currentStack, nextStack, currentRetain, nextRetain, nextRoots, this.currentRootSigs, this.gmssPS, this.digestProvider));
    }
    ...
    private GMSSRootCalc generateCurrentAuthpathAndRoot(byte[] lowerRoot, Vector currentStack, byte[] seed, int h) {
        ...
        GMSSRootCalc treeToConstruct = new GMSSRootCalc(this.heightOfTrees[h], this.f7079K[h], this.digestProvider);
        ...
        for (int i = 1; i < (1 << this.heightOfTrees[h]); i++) {
            if (i == seedForTreehashIndex && count < this.heightOfTrees[h] - this.f7079K[h]) {
    ...
    private void initializeDefault() {
        ...
        initialize(new GMSSKeyGenerationParameters(new SecureRandom(), new GMSSParameters(defh.length, defh, new int[]{3, 3, 3, 3}, new int[]{2, 2, 2, 2})));
    }
