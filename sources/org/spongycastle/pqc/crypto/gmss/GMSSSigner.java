...
public class GMSSSigner implements MessageSigner {
    ...
    private void initSign() {
        ...
        if (gmssPrivateKey.isUsed()) {
            ...
            System.arraycopy(gmssPrivateKey.getCurrentSeeds()[this.numLayer - 1], 0, dummy, 0, this.mdLength);
            ...
            for (int j = 0; j < this.numLayer; j++) {
                this.currentAuthPaths[j] = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{helpCurrentAuthPaths[j].length, this.mdLength});
    ...
    public boolean verifySignature(byte[] message, byte[] signature) {
        ...
        for (int j = this.numLayer - 1; j >= 0; j--) {
            ...
            byte[][] authPath = (byte[][]) Array.newInstance(Byte.TYPE, new int[]{this.gmssPS.getHeightOfTrees()[j], this.mdLength});
            for (byte[] arraycopy : authPath) {
                System.arraycopy(signature, nextEntry, arraycopy, 0, this.mdLength);
