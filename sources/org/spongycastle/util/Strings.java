    ...
    public static String fromUTF8ByteArray(byte[] bytes) {
        ...
        while (i < bytes.length) {
            ...
            if ((bytes[i] & 240) == 240) {
                length++;
                i += 4;
            } else if ((bytes[i] & ISO7816.INS_CREATE_FILE) == 224) {
                i += 3;
            } else if ((bytes[i] & ISO7816.INS_GET_RESPONSE) == 192) {
                i += 2;
            } else {
                i++;
            }
        ...
        while (i2 < bytes.length) {
            if ((bytes[i2] & 240) == 240) {
                int U = (((((bytes[i2] & 3) << 18) | ((bytes[i2 + 1] & 63) << PassportService.SF_DG12)) | ((bytes[i2 + 2] & 63) << 6)) | (bytes[i2 + 3] & 63)) - 65536;
                ...
                i2 += 4;
                length2 = length3;
            } else if ((bytes[i2] & ISO7816.INS_CREATE_FILE) == 224) {
                ch = (char) (((bytes[i2] & 15) << PassportService.SF_DG12) | ((bytes[i2 + 1] & 63) << 6) | (bytes[i2 + 2] & 63));
                i2 += 3;
            } else if ((bytes[i2] & ISO7816.INS_WRITE_BINARY) == 208) {
                ch = (char) (((bytes[i2] & 31) << 6) | (bytes[i2 + 1] & 63));
                i2 += 2;
            } else if ((bytes[i2] & ISO7816.INS_GET_RESPONSE) == 192) {
                ch = (char) (((bytes[i2] & 31) << 6) | (bytes[i2 + 1] & 63));
                i2 += 2;
            } else {
