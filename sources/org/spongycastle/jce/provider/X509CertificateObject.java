...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
    ...
    public void checkValidity(Date date) throws CertificateExpiredException, CertificateNotYetValidException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f6937c.getEndDate().getTime());
        } else if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f6937c.getStartDate().getTime());
        }
    }

    public int getVersion() {
        return this.f6937c.getVersionNumber();
    }

    public BigInteger getSerialNumber() {
        return this.f6937c.getSerialNumber().getValue();
    }

    public Principal getIssuerDN() {
        try {
            return new X509Principal(X500Name.getInstance(this.f6937c.getIssuer().getEncoded()));
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public X500Principal getIssuerX500Principal() {
        try {
            ...
            new ASN1OutputStream(bOut).writeObject(this.f6937c.getIssuer());
    ...
    public Principal getSubjectDN() {
        return new X509Principal(X500Name.getInstance(this.f6937c.getSubject().toASN1Primitive()));
    }

    public X500Principal getSubjectX500Principal() {
        try {
            ByteArrayOutputStream bOut = new ByteArrayOutputStream();
            new ASN1OutputStream(bOut).writeObject(this.f6937c.getSubject());
            return new X500Principal(bOut.toByteArray());
        } catch (IOException e) {
            throw new IllegalStateException("can't encode issuer DN");
        }
    ...
    public Date getNotBefore() {
        return this.f6937c.getStartDate().getDate();
    }

    public Date getNotAfter() {
        return this.f6937c.getEndDate().getDate();
    }

    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f6937c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public byte[] getSignature() {
        return this.f6937c.getSignature().getOctets();
    }
    ...
    public String getSigAlgOID() {
        return this.f6937c.getSignatureAlgorithm().getAlgorithm().getId();
    }

    public byte[] getSigAlgParams() {
        byte[] bArr = null;
        if (this.f6937c.getSignatureAlgorithm().getParameters() == null) {
            return bArr;
        }
        try {
            return this.f6937c.getSignatureAlgorithm().getParameters().toASN1Primitive().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            return bArr;
        }
    }
    ...
    public boolean[] getIssuerUniqueID() {
        DERBitString id = this.f6937c.getTBSCertificate().getIssuerUniqueId();
        if (id == null) {
            return null;
        }
    ...
    public boolean[] getSubjectUniqueID() {
        DERBitString id = this.f6937c.getTBSCertificate().getSubjectUniqueId();
        if (id == null) {
            return null;
        }
        byte[] bytes = id.getBytes();
    ...
    public Set getCriticalExtensionOIDs() {
        if (getVersion() == 3) {
            Set set = new HashSet();
            Extensions extensions = this.f6937c.getTBSCertificate().getExtensions();
            if (extensions != null) {
    ...
    private byte[] getExtensionBytes(String oid) {
        Extensions exts = this.f6937c.getTBSCertificate().getExtensions();
        if (exts != null) {
    ...
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f6937c.getSubjectPublicKeyInfo());
        } catch (IOException e) {
            return null;
        }
    }
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            return this.f6937c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public final void verify(PublicKey key) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, NoSuchProviderException, SignatureException {
        Signature signature;
        String sigName = X509SignatureUtil.getSignatureName(this.f6937c.getSignatureAlgorithm());
        try {
    ...
    private void checkSignature(PublicKey key, Signature signature) throws CertificateException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        if (!isAlgIdEqual(this.f6937c.getSignatureAlgorithm(), this.f6937c.getTBSCertificate().getSignature())) {
            throw new CertificateException("signature algorithm in TBS cert not same as outer cert");
        }
        X509SignatureUtil.setSignatureParameters(signature, this.f6937c.getSignatureAlgorithm().getParameters());
