...
public class EACTags {
    ...
    public static int encodeTag(ASN1ApplicationSpecific spec) {
        ...
        int retValue2 = ((retValue | 31) << 8) | (tag & CertificateBody.profileType);
        for (int tag2 = tag >> 7; tag2 > 0; tag2 >>= 7) {
            retValue2 = (retValue2 | 128) << 8;
            int currentByte = tag2 & CertificateBody.profileType;
        }
        return retValue2;
    }
    ...
    public static int decodeTag(int tag) {
        ...
        for (int i = 24; i >= 0; i -= 8) {
            int currentByte = (tag >> i) & 255;
            if (currentByte != 0) {
                if (multiBytes) {
                    retValue = (retValue << 7) | (currentByte & CertificateBody.profileType);
                } else if ((currentByte & 31) != 31) {
