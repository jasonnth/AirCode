...
class Tnaf {
    ...
    public static BigInteger norm(byte mu, ZTauElement lambda) {
        BigInteger s1 = lambda.f6976u.multiply(lambda.f6976u);
        BigInteger s2 = lambda.f6976u.multiply(lambda.f6977v);
        BigInteger s3 = lambda.f6977v.multiply(lambda.f6977v).shiftLeft(1);
        if (mu == 1) {
    ...
    public static ZTauElement round(SimpleBigDecimal lambda0, SimpleBigDecimal lambda1, byte mu) {
        ...
        if (lambda1.getScale() != lambda0.getScale()) {
            ...
            ZTauElement zTauElement = new ZTauElement(f0.add(BigInteger.valueOf((long) h0)), f1.add(BigInteger.valueOf((long) h1)));
            return zTauElement;
        } else {
            throw new IllegalArgumentException("mu must be 1 or -1");
        }
    ...
    public static byte[] tauAdicNaf(byte mu, ZTauElement lambda) {
        ...
        if (mu == 1 || mu == -1) {
            ...
            int i = 0;
            int length = 0;
            BigInteger r02 = lambda.f6976u;
            BigInteger r1 = lambda.f6977v;
            while (true) {
    ...
    public static ZTauElement partModReduction(BigInteger k, int m, byte a, BigInteger[] s, byte mu, byte c) {
        ...
        return new ZTauElement(k.subtract(d0.multiply(q.f6976u)).subtract(BigInteger.valueOf(2).multiply(s[1]).multiply(q.f6977v)), s[1].multiply(q.f6976u).subtract(s[0].multiply(q.f6977v)));
    }
    ...
    public static AbstractF2m multiplyRTnaf(AbstractF2m p, BigInteger k) {
        ...
        int m = curve.getFieldSize();
        int a = curve.getA().toBigInteger().intValue();
        return multiplyTnaf(p, partModReduction(k, m, (byte) a, curve.getSi(), getMu(a), 10));
    }
    ...
    public static AbstractF2m multiplyFromTnaf(AbstractF2m p, byte[] u) {
        AbstractF2m abstractF2m;
        AbstractF2m q = (AbstractF2m) p.getCurve().getInfinity();
        AbstractF2m pNeg = (AbstractF2m) p.negate();
        int tauCount = 0;
        for (int i = u.length - 1; i >= 0; i--) {
            ...
            if (ui != 0) {
                ...
                q = (AbstractF2m) q2.add(abstractF2m);
            }
        }
        if (tauCount > 0) {
    ...
    public static byte[] tauAdicWNaf(byte mu, ZTauElement lambda, byte width, BigInteger pow2w, BigInteger tw, ZTauElement[] alpha) {
        ...
        if (mu == 1 || mu == -1) {
            int log2Norm = norm(mu, lambda).bitLength();
            byte[] u = new byte[(log2Norm > 30 ? log2Norm + 4 + width : width + ISO7816.INS_MSE)];
            BigInteger pow2wMin1 = pow2w.shiftRight(1);
            BigInteger r0 = lambda.f6976u;
            BigInteger r1 = lambda.f6977v;
            int i = 0;
            while (true) {
                if (r0.equals(ECConstants.ZERO) && r1.equals(ECConstants.ZERO)) {
                ...
                if (r0.testBit(0)) {
                    ...
                    if (s) {
                        r0 = r0.subtract(alpha[uLocal].f6976u);
                        r1 = r1.subtract(alpha[uLocal].f6977v);
                    } else {
                        r0 = r0.add(alpha[uLocal].f6976u);
                        r1 = r1.add(alpha[uLocal].f6977v);
                    }
                } else {
                    u[i] = 0;
                }
