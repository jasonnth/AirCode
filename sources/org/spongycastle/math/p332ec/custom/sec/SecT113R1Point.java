...
public class SecT113R1Point extends AbstractF2m {
        ...
        if (!X1.isZero()) {
            ...
            if (!B.isZero()) {
                if (X2.isZero()) {
                    ...
                    if (X3.isZero()) {
                        SecT113R1Point secT113R1Point = new SecT113R1Point(curve, X3, curve.getB().sqrt(), this.withCompression);
                        return secT113R1Point;
                    }
                    ...
                    X3 = AU1.multiply(AU2);
                    if (X3.isZero()) {
                        SecT113R1Point secT113R1Point2 = new SecT113R1Point(curve, X3, curve.getB().sqrt(), this.withCompression);
                        return secT113R1Point2;
                    }
    ...
    public ECPoint twice() {
        ...
        if (T.isZero()) {
            return new SecT113R1Point(curve, T, curve.getB().sqrt(), this.withCompression);
        }
        ECFieldElement X3 = T.square();
        ECFieldElement Z3 = Z1IsOne ? T : T.multiply(Z1Sq);
        return new SecT113R1Point(curve, X3, (Z1IsOne ? X1 : X1.multiply(Z1)).squarePlusProduct(T, L1Z1).add(X3).add(Z3), new ECFieldElement[]{Z3}, this.withCompression);
    }
    ...
    public ECPoint twicePlus(ECPoint b) {
        ...
        if (B.isZero()) {
            ...
            SecT113R1Point secT113R1Point = new SecT113R1Point(curve, A, curve.getB().sqrt(), this.withCompression);
            return secT113R1Point;
        } else {
            ECFieldElement X3 = A.square().multiply(X2Z1Sq);
            ECFieldElement Z3 = A.multiply(B).multiply(Z1Sq);
            return new SecT113R1Point(curve, X3, A.add(B).square().multiplyPlusProduct(T, L2plus1, Z3), new ECFieldElement[]{Z3}, this.withCompression);
        }
    }
    ...
    public ECPoint negate() {
        ...
        ECFieldElement L = this.f6974y;
        ECFieldElement Z = this.f6975zs[0];
        return new SecT113R1Point(this.curve, X, L.add(Z), new ECFieldElement[]{Z}, this.withCompression);
    }
}
