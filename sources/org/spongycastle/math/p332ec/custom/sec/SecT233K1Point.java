...
/* renamed from: org.spongycastle.math.ec.custom.sec.SecT233K1Point */
public class SecT233K1Point extends AbstractF2m {
        ...
        if (!X1.isZero()) {
            ...
            if (!B.isZero()) {
                if (X2.isZero()) {
                    ...
                    if (X3.isZero()) {
                        SecT233K1Point secT233K1Point = new SecT233K1Point(curve, X3, curve.getB(), this.withCompression);
                        return secT233K1Point;
                    }
                    ...
                    X3 = AU1.multiply(AU2);
                    if (X3.isZero()) {
                        SecT233K1Point secT233K1Point2 = new SecT233K1Point(curve, X3, curve.getB(), this.withCompression);
                        return secT233K1Point2;
                    }
    ...
    public ECPoint twice() {
        ...
        if (T.isZero()) {
            return new SecT233K1Point(curve, T, curve.getB(), this.withCompression);
        }
        ECFieldElement X3 = T.square();
        ECFieldElement Z3 = Z1IsOne ? T : T.multiply(Z1Sq);
        ECFieldElement t1 = L1.add(X1).square();
        return new SecT233K1Point(curve, X3, t1.add(T).add(Z1Sq).multiply(t1).add(Z1IsOne ? Z1 : Z1Sq.square()).add(X3).add(Z3), new ECFieldElement[]{Z3}, this.withCompression);
    }
    ...
    public ECPoint twicePlus(ECPoint b) {
        ...
        if (B.isZero()) {
            ...
            SecT233K1Point secT233K1Point = new SecT233K1Point(curve, A, curve.getB(), this.withCompression);
            return secT233K1Point;
        } else {
            ECFieldElement X3 = A.square().multiply(X2Z1Sq);
            ECFieldElement Z3 = A.multiply(B).multiply(Z1Sq);
            return new SecT233K1Point(curve, X3, A.add(B).square().multiplyPlusProduct(T, L2plus1, Z3), new ECFieldElement[]{Z3}, this.withCompression);
        }
    }
    ...
    public ECPoint negate() {
        ...
        ECFieldElement L = this.f6974y;
        ECFieldElement Z = this.f6975zs[0];
        return new SecT233K1Point(this.curve, X, L.add(Z), new ECFieldElement[]{Z}, this.withCompression);
    }
}
