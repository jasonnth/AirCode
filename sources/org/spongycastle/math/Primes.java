...
public abstract class Primes {
    ...
    private static boolean implHasAnySmallFactors(BigInteger x) {
        ...
        if (r6 % 107 == 0 || r6 % 109 == 0 || r6 % 113 == 0 || r6 % CertificateBody.profileType == 0) {
            return true;
        }
        int r7 = x.mod(BigInteger.valueOf((long) 371700317)).intValue();
        if (r7 % ISO781611.CREATION_DATE_AND_TIME_TAG == 0 || r7 % CipherSuite.TLS_DH_anon_WITH_CAMELLIA_256_CBC_SHA == 0 || r7 % CipherSuite.TLS_PSK_WITH_3DES_EDE_CBC_SHA == 0 || r7 % 149 == 0) {
            return true;
        }
        int r8 = x.mod(BigInteger.valueOf((long) 645328247)).intValue();
        if (r8 % CipherSuite.TLS_DH_DSS_WITH_SEED_CBC_SHA == 0 || r8 % CipherSuite.TLS_RSA_WITH_AES_256_GCM_SHA384 == 0 || r8 % CipherSuite.TLS_DHE_DSS_WITH_AES_256_GCM_SHA384 == 0 || r8 % CipherSuite.TLS_DH_anon_WITH_AES_256_GCM_SHA384 == 0) {
            return true;
        }
        int r9 = x.mod(BigInteger.valueOf((long) 1070560157)).intValue();
        if (r9 % CipherSuite.TLS_RSA_PSK_WITH_AES_256_GCM_SHA384 == 0 || r9 % CipherSuite.TLS_DHE_PSK_WITH_AES_256_CBC_SHA384 == 0 || r9 % CipherSuite.TLS_DHE_PSK_WITH_NULL_SHA384 == 0 || r9 % CipherSuite.TLS_DH_anon_WITH_CAMELLIA_128_CBC_SHA256 == 0) {
            return true;
        }
        int r10 = x.mod(BigInteger.valueOf((long) 1596463769)).intValue();
        if (r10 % CipherSuite.TLS_DH_DSS_WITH_CAMELLIA_256_CBC_SHA256 == 0 || r10 % CipherSuite.TLS_DH_anon_WITH_CAMELLIA_256_CBC_SHA256 == 0 || r10 % 199 == 0 || r10 % SMALL_FACTOR_LIMIT == 0) {
            return true;
        }
        return false;
    }
    ...
    private static STOutput implSTRandomPrime(Digest d, int length, byte[] primeSeed) {
        ...
        if (length < 33) {
            ...
            do {
                ...
                if (isPrime32(c64)) {
                    STOutput sTOutput = new STOutput(BigInteger.valueOf(c64), primeSeed, primeGenCounter);
                    return sTOutput;
                }
        ...
        BigInteger x = hashGen(d, primeSeed2, iterations + 1).mod(ONE.shiftLeft(length - 1)).setBit(length - 1);
        ...
        while (true) {
            ...
            if (!implHasAnySmallFactors(c)) {
                BigInteger a = hashGen(d, primeSeed2, iterations + 1).mod(c.subtract(THREE)).add(TWO);
                ...
                if (c.gcd(z.subtract(ONE)).equals(ONE) && z.modPow(c02, c).equals(ONE)) {
                    STOutput sTOutput2 = new STOutput(c, primeSeed2, primeGenCounter2);
                    return sTOutput2;
                }
            } else {
                inc(primeSeed2, iterations + 1);
            }
            if (primeGenCounter2 >= (length * 4) + oldCounter) {
                throw new IllegalStateException("Too many iterations in Shawe-Taylor Random_Prime Routine");
            }
            dt += 2;
            c = c.add(c0x2);
