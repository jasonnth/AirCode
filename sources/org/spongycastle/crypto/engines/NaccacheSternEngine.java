        ...
        if (this.key == null) {
            ...
            for (int i = 0; i < primes.size(); i++) {
                ...
                if (this.lookup[i].size() != ((BigInteger) primes.elementAt(i)).intValue()) {
                    ...
                    InvalidCipherTextException invalidCipherTextException = new InvalidCipherTextException("Error in lookup Array for " + ((BigInteger) primes.elementAt(i)).intValue() + ": Size mismatch. Expected ArrayList with length " + ((BigInteger) primes.elementAt(i)).intValue() + " but found ArrayList of length " + this.lookup[i].size());
