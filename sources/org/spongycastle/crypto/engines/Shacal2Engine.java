...
public class Shacal2Engine implements BlockCipher {
    ...
    private void encryptBlock(byte[] in, int inOffset, byte[] out, int outOffset) {
        ...
        for (int i = 0; i < 64; i++) {
            int tmp = ((((block[4] >>> 6) | (block[4] << -6)) ^ ((block[4] >>> 11) | (block[4] << -11))) ^ ((block[4] >>> 25) | (block[4] << -25))) + ((block[4] & block[5]) ^ ((block[4] ^ -1) & block[6])) + block[7] + f6709K[i] + this.workingKey[i];
            block[7] = block[6];
            block[6] = block[5];
            block[5] = block[4];
            block[4] = block[3] + tmp;
    ...
    private void decryptBlock(byte[] in, int inOffset, byte[] out, int outOffset) {
        ...
        for (int i = 63; i > -1; i--) {
            ...
            block[5] = block[6];
            block[6] = block[7];
            block[7] = (((tmp - f6709K[i]) - this.workingKey[i]) - ((((block[4] >>> 6) | (block[4] << -6)) ^ ((block[4] >>> 11) | (block[4] << -11))) ^ ((block[4] >>> 25) | (block[4] << -25)))) - ((block[4] & block[5]) ^ ((block[4] ^ -1) & block[6]));
        }
        ints2bytes(block, out, outOffset);
    }
