...
public class CramerShoupCoreEngine {
    ...
    public CramerShoupCiphertext encryptBlock(BigInteger input) {
        ...
        if (!this.key.isPrivate() && this.forEncryption && (this.key instanceof CramerShoupPublicKeyParameters)) {
            ...
            result = new CramerShoupCiphertext(u1, u2, e, pk.getC().modPow(r, p).multiply(pk.getD().modPow(r.multiply(a), p)).mod(p));
        }
        return result;
    }
    ...
    public BigInteger decryptBlock(CramerShoupCiphertext input) throws CramerShoupCiphertextException {
        ...
        digest.doFinal(out, 0);
        BigInteger a = new BigInteger(1, out);
        if (input.f6670v.equals(input.f6668u1.modPow(sk.getX1().add(sk.getY1().multiply(a)), p).multiply(input.f6669u2.modPow(sk.getX2().add(sk.getY2().multiply(a)), p)).mod(p))) {
            return input.f6667e.multiply(input.f6668u1.modPow(sk.getZ(), p).modInverse(p)).mod(p);
        }
