...
public class OCBBlockCipher implements AEADBlockCipher {
    ...
    public void init(boolean forEncryption2, CipherParameters parameters) throws IllegalArgumentException {
        ...
        this.f6790L = new Vector();
        this.f6790L.addElement(OCB_double(this.L_Dollar));
    ...
    public int processNonce(byte[] N) {
        ...
        nonce[15] = (byte) (nonce[15] & ISO7816.INS_GET_RESPONSE);
    ...
    public byte[] getLSub(int n) {
        while (n >= this.f6790L.size()) {
            this.f6790L.addElement(OCB_double((byte[]) this.f6790L.lastElement()));
        }
        return (byte[]) this.f6790L.elementAt(n);
    }
    ...
    protected static void OCB_extend(byte[] block, int pos) {
        block[pos] = ISOFileInfo.DATA_BYTES1;
        while (true) {
            pos++;
            if (pos < 16) {
                block[pos] = 0;
