...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _P */
    private ECPoint f6878_P;

    /* renamed from: _Q */
    private ECPoint f6879_Q;
    private C5397Fp _curve;
    ...
    /* renamed from: _s */
    private byte[] f6880_s;
    private int _sLength;
    private int _securityStrength;
    private int _seedlen;

    static {
        C5397Fp curve = (C5397Fp) NISTNamedCurves.getByName("P-256").getCurve();
        nistPoints[0] = new DualECPoints(128, curve.createPoint(p256_Px, p256_Py), curve.createPoint(p256_Qx, p256_Qy), 1);
        C5397Fp curve2 = (C5397Fp) NISTNamedCurves.getByName("P-384").getCurve();
        nistPoints[1] = new DualECPoints(192, curve2.createPoint(p384_Px, p384_Py), curve2.createPoint(p384_Qx, p384_Qy), 1);
        C5397Fp curve3 = (C5397Fp) NISTNamedCurves.getByName("P-521").getCurve();
        nistPoints[2] = new DualECPoints(256, curve3.createPoint(p521_Px, p521_Py), curve3.createPoint(p521_Qx, p521_Qy), 1);
    }
    ...
    public DualECSP800DRBG(DualECPoints[] pointSet, Digest digest, int securityStrength, EntropySource entropySource, byte[] personalizationString, byte[] nonce) {
        ...
        if (Utils.isTooLarge(personalizationString, 512)) {
        ...
        } else if (entropySource.entropySize() < securityStrength || entropySource.entropySize() > 4096) {
            ...
            while (true) {
                if (i == pointSet.length) {
                    break;
                } else if (securityStrength > pointSet[i].getSecurityStrength()) {
                    i++;
                } else if (Utils.getMaxSecurityStrength(digest) < pointSet[i].getSecurityStrength()) {
                    throw new IllegalArgumentException("Requested security strength is not supported by digest");
                } else {
                    this._seedlen = pointSet[i].getSeedLen();
                    this._outlen = pointSet[i].getMaxOutlen() / 8;
                    this.f6878_P = pointSet[i].getP();
                    this.f6879_Q = pointSet[i].getQ();
                }
            }
            if (this.f6878_P == null) {
                throw new IllegalArgumentException("security strength cannot be greater than 256 bits");
            }
            this.f6880_s = Utils.hash_df(this._digest, seedMaterial, this._seedlen);
            this._sLength = this.f6880_s.length;
    ...
    public int generate(byte[] output, byte[] additionalInput, boolean predictionResistant) {
        ...
        if (Utils.isTooLarge(additionalInput, 512)) {
        ...
        } else if (this._reseedCounter + ((long) m) > RESEED_MAX) {
            return -1;
        } else {
            ...
            if (additionalInput != null) {
                s = new BigInteger(1, xor(this.f6880_s, Utils.hash_df(this._digest, additionalInput, this._seedlen)));
            } else {
                s = new BigInteger(1, this.f6880_s);
            }
            Arrays.fill(output, 0);
            int outOffset = 0;
            for (int i = 0; i < m; i++) {
                s = getScalarMultipleXCoord(this.f6878_P, s);
                byte[] r = getScalarMultipleXCoord(this.f6879_Q, s).toByteArray();
                if (r.length > this._outlen) {
                    System.arraycopy(r, r.length - this._outlen, output, outOffset, this._outlen);
                } else {
                    System.arraycopy(r, 0, output, (this._outlen - r.length) + outOffset, r.length);
                }
                outOffset += this._outlen;
                this._reseedCounter++;
            }
            if (outOffset < output.length) {
                s = getScalarMultipleXCoord(this.f6878_P, s);
                byte[] r2 = getScalarMultipleXCoord(this.f6879_Q, s).toByteArray();
                int required = output.length - outOffset;
                if (r2.length > this._outlen) {
                    System.arraycopy(r2, r2.length - this._outlen, output, outOffset, required);
                } else {
                    System.arraycopy(r2, 0, output, (this._outlen - r2.length) + outOffset, required);
                }
                this._reseedCounter++;
            }
            this.f6880_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f6878_P, s));
            return numberOfBits;
        }
    }
    ...
    public void reseed(byte[] additionalInput) {
        ...
        this.f6880_s = Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f6880_s, this._seedlen), getEntropy(), additionalInput), this._seedlen);
        this._reseedCounter = 0;
    }
    ...
    private BigInteger getScalarMultipleXCoord(ECPoint p, BigInteger s) {
        return this._fixedPointMultiplier.multiply(p, s).normalize().getAffineXCoord().toBigInteger();
    }
}
