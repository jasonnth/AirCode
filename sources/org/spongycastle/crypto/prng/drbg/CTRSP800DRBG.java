...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private void CTR_DRBG_Instantiate_algorithm(byte[] entropy, byte[] nonce, byte[] personalisationString) {
        ...
        this.f6875_V = new byte[outlen];
        CTR_DRBG_Update(seed, this._Key, this.f6875_V);
    ...
    private void CTR_DRBG_Reseed_algorithm(byte[] additionalInput) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(getEntropy(), additionalInput), this._seedLength), this._Key, this.f6875_V);
        this._reseedCounter = 1;
    }
    ...
    public int generate(byte[] output, byte[] additionalInput, boolean predictionResistant) {
        ...
        if (additionalInput != null) {
            ...
            CTR_DRBG_Update(additionalInput2, this._Key, this.f6875_V);
        } else {
            additionalInput2 = new byte[this._seedLength];
        }
        byte[] out = new byte[this.f6875_V.length];
        ...
        for (int i = 0; i <= output.length / out.length; i++) {
            if (output.length - (out.length * i) > out.length) {
                ...
                bytesToCopy = output.length - (this.f6875_V.length * i);
            }
            if (bytesToCopy != 0) {
                addOneTo(this.f6875_V);
                this._engine.processBlock(this.f6875_V, 0, out, 0);
    ...
    private void padKey(byte[] keyMaster, int keyOff, byte[] tmp, int tmpOff) {
        ...
        tmp[tmpOff + 4] = (byte) ((keyMaster[keyOff + 3] << 4) | ((keyMaster[keyOff + 4] & ISO7816.INS_CREATE_FILE) >>> 4));
        tmp[tmpOff + 5] = (byte) ((keyMaster[keyOff + 4] << 3) | ((keyMaster[keyOff + 5] & ISO7816.INS_GET_RESPONSE) >>> 5));
        tmp[tmpOff + 6] = (byte) ((keyMaster[keyOff + 5] << 2) | ((keyMaster[keyOff + 6] & ISOFileInfo.DATA_BYTES1) >>> 6));
