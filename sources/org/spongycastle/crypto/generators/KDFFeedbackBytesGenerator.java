...
public class KDFFeedbackBytesGenerator implements MacDerivationFunction {
    ...
    /* renamed from: iv */
    private byte[] f6736iv;

    /* renamed from: k */
    private byte[] f6737k = new byte[this.f6735h];
    private int maxSizeExcl;
    private final Mac prf;
    private boolean useCounter;

    public KDFFeedbackBytesGenerator(Mac prf2) {
        this.prf = prf2;
        this.f6735h = prf2.getMacSize();
    }
    ...
    public void init(DerivationParameters params) {
        ...
        if (feedbackParams.useCounter()) {
            BigInteger maxSize = TWO.pow(r).multiply(BigInteger.valueOf((long) this.f6735h));
        ...
        this.f6736iv = feedbackParams.getIV();
    ...
    public int generateBytes(byte[] out, int outOff, int len) throws DataLengthException, IllegalArgumentException {
        ...
        if (this.generatedBytes % this.f6735h == 0) {
            generateNext();
        }
        int toGenerate = len;
        int posInK = this.generatedBytes % this.f6735h;
        int toCopy = Math.min(this.f6735h - (this.generatedBytes % this.f6735h), toGenerate);
        System.arraycopy(this.f6737k, posInK, out, outOff, toCopy);
        this.generatedBytes += toCopy;
        int toGenerate2 = toGenerate - toCopy;
        int outOff2 = outOff + toCopy;
        while (toGenerate2 > 0) {
            generateNext();
            int toCopy2 = Math.min(this.f6735h, toGenerate2);
            System.arraycopy(this.f6737k, 0, out, outOff2, toCopy2);
