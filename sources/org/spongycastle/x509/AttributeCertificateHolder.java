...
public class AttributeCertificateHolder implements CertSelector, Selector {
    ...
    public AttributeCertificateHolder(X509Principal issuerName, BigInteger serialNumber) {
        this.holder = new Holder(new IssuerSerial(GeneralNames.getInstance(new DERSequence((ASN1Encodable) new GeneralName((X509Name) issuerName))), new ASN1Integer(serialNumber)));
    }
    ...
    private GeneralNames generateGeneralNames(X509Principal principal) {
        return GeneralNames.getInstance(new DERSequence((ASN1Encodable) new GeneralName((X509Name) principal)));
    }
    ...
    public boolean match(Certificate cert) {
        ...
        try {
            if (this.holder.getBaseCertificateID() != null) {
                if (!this.holder.getBaseCertificateID().getSerial().getValue().equals(x509Cert.getSerialNumber()) || !matchesDN(PrincipalUtil.getIssuerX509Principal(x509Cert), this.holder.getBaseCertificateID().getIssuer())) {
                    z = false;
                }
                return z;
                ...
                try {
                    ...
                    return !Arrays.areEqual(md.digest(), getObjectDigest()) ? false : false;
                } catch (Exception e) {
                    return false;
                }
            }
