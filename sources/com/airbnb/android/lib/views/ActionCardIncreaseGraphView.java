...
public class ActionCardIncreaseGraphView extends View {
    @BindColor
    int babu;
    @BindColor
    int babuFilled;
    ...
    public ActionCardIncreaseGraphView(Context context) {
        super(context);
        init();
    }

    public ActionCardIncreaseGraphView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ActionCardIncreaseGraphView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setIncreaseValueAndCaption(Insight story) {
        this.increaseValue = story.getGraphicPayload().getSegmentedBoostBarValue();
        this.increaseValueText = getResources().getString(C0880R.string.insight_increase_percent, new Object[]{Integer.valueOf(this.increaseValue)});
        this.increaseValueTextWidth = this.graphTextPaint.measureText(this.increaseValueText);
        this.graphTextPaint.getTextBounds(this.increaseValueText, 0, this.increaseValueText.length(), this.textRect);
        this.increaseValueCenterPadding = this.barGraphHeight - (this.textRect.height() / 2);
        this.graphCaption = story.getCopies().getGraphicCaption();
        invalidate();
    }
