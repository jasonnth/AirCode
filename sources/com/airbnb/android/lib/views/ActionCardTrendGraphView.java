...
public class ActionCardTrendGraphView extends View {
    private final float GRAPH_HEIGHT_FACTOR = 0.6f;
    ...
    public ActionCardTrendGraphView(Context context) {
        super(context);
        init();
    }

    public ActionCardTrendGraphView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ActionCardTrendGraphView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        this.pageViewsGraphPaint.setColor(ContextCompat.getColor(getContext(), C0880R.color.n2_babu));
        ...
        this.monthTickMarkPaint.setColor(ContextCompat.getColor(getContext(), C0880R.color.c_foggy_light));
        ...
        this.monthTextPaint.setTextSize((float) getResources().getDimensionPixelSize(C0880R.dimen.action_card_month_text_size));
        this.monthTextPaint.setTypeface(FontManager.getTypeface(Font.CircularBook, getContext()));
    }

    public void setMonthlyMarketDemands(List<ActionCardMonthlyMarketDemand> monthlyMarketDemands2) {
        this.monthlyMarketDemands.clear();
        this.monthlyMarketDemands.addAll(monthlyMarketDemands2);
        setMonthText();
        setTextHeight();
        setMinAndMaxValues();
        invalidate();
    ...
    private void setTextHeight() {
        this.monthTextPaint.getTextBounds(this.firstMonthText, 0, this.firstMonthText.length(), this.textRect);
        this.textHeight = this.textRect.height();
    }

    private void setMonthText() {
        if (!this.monthlyMarketDemands.isEmpty()) {
            AirDate startDate = ((ActionCardMonthlyMarketDemand) this.monthlyMarketDemands.get(0)).getDate();
            String dateFormat = "MMM";
            this.firstMonthText = startDate.formatDate(dateFormat);
            this.secondMonthText = startDate.plusMonths(1).formatDate(dateFormat);
            this.thirdMonthText = startDate.plusMonths(2).formatDate(dateFormat);
        }
    }

    private void setMinAndMaxValues() {
        this.minValue = ((ActionCardMonthlyMarketDemand) this.monthlyMarketDemands.get(0)).getPageViews();
        this.maxValue = ((ActionCardMonthlyMarketDemand) this.monthlyMarketDemands.get(0)).getPageViews();
        for (ActionCardMonthlyMarketDemand demand : this.monthlyMarketDemands) {
            float value = demand.getPageViews();
    ...
    public void onDraw(Canvas canvas) {
        ...
        this.viewHeight = getHeight() - (this.padding * 2);
        this.graphHeight = (((this.viewHeight - this.tickHeight) - this.textHeight) - (this.padding * 2)) - this.graphAndTickPadding;
    ...
    private int getYValue(int position) {
        return (int) (((float) this.graphHeight) - (((((ActionCardMonthlyMarketDemand) this.monthlyMarketDemands.get(position)).getPageViews() - this.minValue) * this.yNormalizationValue) * 0.6f));
    }
