...
public class zzy extends Thread implements OnFrameAvailableListener, zza {
    ...
    private int zziq() {
        int zzc = zzc(35633, zzit());
        if (zzc == 0) {
            return 0;
        }
        int zzc2 = zzc(35632, zziu());
        if (zzc2 == 0) {
            return 0;
        }
    ...
    private String zzit() {
        ...
        return !((String) zzfz.get()).equals(zzfz.zzfr()) ? (String) zzfz.get() : "attribute highp vec3 aPosition;varying vec3 pos;void main() {  gl_Position = vec4(aPosition, 1.0);  pos = aPosition;}";
    }

    private String zziu() {
        zzfz<String> zzfz = zzgd.zzDp;
        return !((String) zzfz.get()).equals(zzfz.zzfr()) ? (String) zzfz.get() : "#extension GL_OES_EGL_image_external : require\n#define INV_PI 0.3183\nprecision highp float;varying vec3 pos;uniform samplerExternalOES uSplr;uniform mat3 uVMat;uniform float uFOVx;uniform float uFOVy;void main() {  vec3 ray = vec3(pos.x * tan(uFOVx), pos.y * tan(uFOVy), -1);  ray = (uVMat * ray).xyz;  ray = normalize(ray);  vec2 texCrd = vec2(    0.5 + atan(ray.x, - ray.z) * INV_PI * 0.5, acos(ray.y) * INV_PI);  gl_FragColor = vec4(texture2D(uSplr, texCrd).xyz, 1.0);}";
    }
    ...
    public void run() {
        boolean z = true;
        if (this.zzOI == null) {
            zzpk.m1280e("SphericalVideoProcessor started with no output texture.");
            this.zzON.countDown();
            return;
        }
        boolean zzir = zzir();
        ...
        if (!zzir || !z) {
            ...
            zzpk.m1280e(str2);
            zzw.zzcQ().zza(new Throwable(str2), "SphericalVideoProcessor.run.1");
            zziv();
            this.zzON.countDown();
            return;
        }
    ...
    public boolean zzir() {
        ...
        this.zzOS = this.zzOP.eglCreateWindowSurface(this.zzOQ, zzis, this.zzOI, null);
