...
public class zzzq {
    ...
    public static long zzn(byte[] bArr) {
        ...
        long zzp = bArr.length <= 32 ? zza(bArr, -1397348546323613475L) : bArr.length <= 64 ? zzo(bArr) : zzp(bArr);
        long j2 = bArr.length >= 8 ? zzb(bArr, 0) : -6505348102511208375L;
        if (bArr.length >= 9) {
            j = zzb(bArr, bArr.length - 8);
        }
        long zzc = zzc(zzp + j, j2);
    ...
    private static long zzo(byte[] bArr) {
        ...
        return zzI((zzI(((Long.rotateRight(zzb8, 31) + rotateRight5 + rotateRight6 + Long.rotateRight(zzb7, 7) + j) * -4288712594273399085L) + ((zzb8 + zzb6 + rotateRight4) * -6505348102511208375L)) * -6505348102511208375L) + rotateRight4) * -4288712594273399085L;
    }
    ...
    private static long zzp(byte[] bArr) {
        ...
        int i = (length - 1) & -64;
        int i2 = 0;
        long rotateRight2 = -8261664234251669945L * Long.rotateRight(zzI + zzb, 39);
        while (true) {
            int i3 = i;
            long rotateRight3 = Long.rotateRight(rotateRight2 + rotateRight + jArr[0] + zzb(bArr, i2 + 16), 37) * -8261664234251669945L;
            long j = rotateRight3 ^ jArr2[1];
            rotateRight = (Long.rotateRight((jArr[1] + rotateRight) + zzb(bArr, i2 + 48), 42) * -8261664234251669945L) ^ jArr[0];
            long rotateRight4 = Long.rotateRight(zzI ^ jArr2[0], 33);
            zza(bArr, i2, jArr[1] * -8261664234251669945L, jArr2[0] + j, jArr);
            zza(bArr, i2 + 32, rotateRight4 + jArr2[1], rotateRight, jArr2);
            i2 += 64;
            i = i3 - 64;
            if (i == 0) {
                return zzc(zzc(jArr[0], jArr2[0]) + (zzI(rotateRight) * -8261664234251669945L) + j, zzc(jArr[1], jArr2[1]) + rotateRight4);
            }
            zzI = j;
