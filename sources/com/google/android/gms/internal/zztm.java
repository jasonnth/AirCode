...
public class zztm {
    ...
    public static zzrl zza(zztd zztd, String str) {
        ...
        try {
            ...
            Map zza = zzo.zza(new URI(valueOf.length() != 0 ? str2.concat(valueOf) : new String(str2)), JPushConstants.ENCODING_UTF_8);
            zzrl zzrl = new zzrl();
            zzrl.zzbu((String) zza.get("utm_content"));
            zzrl.zzbs((String) zza.get("utm_medium"));
            zzrl.setName((String) zza.get("utm_campaign"));
            zzrl.zzbr((String) zza.get("utm_source"));
            zzrl.zzbt((String) zza.get("utm_term"));
            zzrl.zzbv((String) zza.get("utm_id"));
            zzrl.zzbw((String) zza.get("anid"));
            zzrl.zzbx((String) zza.get("gclid"));
            zzrl.zzby((String) zza.get("dclid"));
            zzrl.zzbz((String) zza.get("aclid"));
            return zzrl;
    ...
    public static String zzcg(String str) {
        ...
        if (str.contains("%3D")) {
            try {
                str = URLDecoder.decode(str, JPushConstants.ENCODING_UTF_8);
        ...
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < 11; i++) {
            if (!TextUtils.isEmpty((CharSequence) zzce.get(strArr[i]))) {
                if (sb.length() > 0) {
                    sb.append("&");
                }
                sb.append(strArr[i]).append("=").append((String) zzce.get(strArr[i]));
            }
        }
        return sb.toString();
    }
    ...
    public static boolean zzg(String str, boolean z) {
        ...
        if (str.equalsIgnoreCase(InternalLogger.EVENT_PARAM_EXTRAS_FALSE) || str.equalsIgnoreCase("no") || str.equalsIgnoreCase(AppEventsConstants.EVENT_PARAM_VALUE_NO)) {
            return false;
        }
        return z;
    }
