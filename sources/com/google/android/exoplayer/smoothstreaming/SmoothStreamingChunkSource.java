...
public class SmoothStreamingChunkSource implements ChunkSource, Output {
    ...
    public SmoothStreamingChunkSource(ManifestFetcher<SmoothStreamingManifest> manifestFetcher2, SmoothStreamingTrackSelector trackSelector2, DataSource dataSource2, FormatEvaluator adaptiveFormatEvaluator2, long liveEdgeLatencyMs) {
        this(manifestFetcher2, (SmoothStreamingManifest) manifestFetcher2.getManifest(), trackSelector2, dataSource2, adaptiveFormatEvaluator2, liveEdgeLatencyMs);
    }
    ...
    private SmoothStreamingChunkSource(ManifestFetcher<SmoothStreamingManifest> manifestFetcher2, SmoothStreamingManifest initialManifest, SmoothStreamingTrackSelector trackSelector2, DataSource dataSource2, FormatEvaluator adaptiveFormatEvaluator2, long liveEdgeLatencyMs) {
        ...
        if (protectionElement != null) {
            ...
            this.drmInitData.put(protectionElement.uuid, new SchemeInitData("video/mp4", protectionElement.data));
            return;
        }
    ...
    private MediaFormat initManifestTrack(SmoothStreamingManifest manifest, int elementIndex, int trackIndex) {
        ...
        MediaFormat mediaFormat2 = (MediaFormat) this.mediaFormats.get(manifestTrackKey);
        ...
        switch (element.type) {
            case 0:
                ...
                mediaFormat = MediaFormat.createAudioFormat(format.f3148id, format.mimeType, format.bitrate, -1, durationUs, format.audioChannels, format.audioSamplingRate, csd, format.language);
                mp4TrackType = Track.TYPE_soun;
                break;
            case 1:
                mediaFormat = MediaFormat.createVideoFormat(format.f3148id, format.mimeType, format.bitrate, -1, durationUs, format.width, format.height, Arrays.asList(csdArray));
                mp4TrackType = Track.TYPE_vide;
                break;
            case 2:
                mediaFormat = MediaFormat.createTextFormat(format.f3148id, format.mimeType, format.bitrate, durationUs, format.language);
    ...
    private static int getManifestTrackIndex(StreamElement element, Format format) {
        TrackElement[] tracks2 = element.tracks;
