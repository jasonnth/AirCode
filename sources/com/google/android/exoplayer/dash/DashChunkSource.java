...
import com.google.android.exoplayer.C4016TimeRange.DynamicTimeRange;
import com.google.android.exoplayer.C4016TimeRange.StaticTimeRange;
...
public class DashChunkSource implements ChunkSource, Output {
    ...
    private C4016TimeRange availableRange;
    ...
    protected static final class PeriodHolder {
        ...
        public PeriodHolder(int localIndex2, MediaPresentationDescription manifest, int manifestIndex, ExposedTrack selectedTrack) {
            ...
            if (!selectedTrack.isAdaptive()) {
                this.representationIndices = new int[]{getRepresentationIndex(representations, selectedTrack.fixedFormat.f3148id)};
            } else {
                ...
                for (int j = 0; j < selectedTrack.adaptiveFormats.length; j++) {
                    this.representationIndices[j] = getRepresentationIndex(representations, selectedTrack.adaptiveFormats[j].f3148id);
                }
            }
            ...
            for (int i : this.representationIndices) {
                ...
                this.representationHolders.put(representation.format.f3148id, new RepresentationHolder(this.startTimeUs, periodDurationUs, representation));
            }
        ...
        public void updatePeriod(MediaPresentationDescription manifest, int manifestIndex, ExposedTrack selectedTrack) throws BehindLiveWindowException {
            ...
            for (int i : this.representationIndices) {
                Representation representation = (Representation) representations.get(i);
                ((RepresentationHolder) this.representationHolders.get(representation.format.f3148id)).updateRepresentation(periodDurationUs, representation);
            }
            updateRepresentationIndependentProperties(periodDurationUs, (Representation) representations.get(this.representationIndices[0]));
        }
        ...
        private static int getRepresentationIndex(List<Representation> representations, String formatId) {
            for (int i = 0; i < representations.size(); i++) {
                if (formatId.equals(((Representation) representations.get(i)).format.f3148id)) {
                    return i;
                }
            }
    ...
    public DashChunkSource(ManifestFetcher<MediaPresentationDescription> manifestFetcher2, DashTrackSelector trackSelector2, DataSource dataSource2, FormatEvaluator adaptiveFormatEvaluator2, long liveEdgeLatencyMs, long elapsedRealtimeOffsetMs, Handler eventHandler2, EventListener eventListener2, int eventSourceId2) {
        this(manifestFetcher2, (MediaPresentationDescription) manifestFetcher2.getManifest(), trackSelector2, dataSource2, adaptiveFormatEvaluator2, new SystemClock(), liveEdgeLatencyMs * 1000, elapsedRealtimeOffsetMs * 1000, true, eventHandler2, eventListener2, eventSourceId2);
    }
        ...
        if (selectedFormat == null) {
            ...
            if (queue.isEmpty()) {
                ...
                if (this.live && nextSegmentStartTimeUs < this.availableRangeValues[0]) {
                    ...
                    if (previous.parentId != lastPeriodHolder.localIndex || !((RepresentationHolder) lastPeriodHolder.representationHolders.get(previous.format.f3148id)).isBeyondLastSegment(previous.getNextChunkIndex())) {
                        ...
                        if (periodHolder == null) {
                            ...
                            startingNewPeriod = true;
                        } else if (!periodHolder.isIndexUnbounded() && ((RepresentationHolder) periodHolder.representationHolders.get(previous.format.f3148id)).isBeyondLastSegment(previous.getNextChunkIndex())) {
            ...
            RepresentationHolder representationHolder = (RepresentationHolder) periodHolder.representationHolders.get(selectedFormat.f3148id);
    ...
    public void onChunkLoadCompleted(Chunk chunk) {
        if (chunk instanceof InitializationChunk) {
            ...
            String formatId = initializationChunk.format.f3148id;
    ...
    public void fixedTrack(MediaPresentationDescription manifest, int periodIndex, int adaptationSetIndex, int representationIndex) {
        ...
        if (mediaMimeType == null) {
            Log.w("DashChunkSource", "Skipped track " + representationFormat.f3148id + " (unknown media mime type)");
            return;
        }
        ...
        if (trackFormat == null) {
            Log.w("DashChunkSource", "Skipped track " + representationFormat.f3148id + " (unknown media format)");
        } else {
    ...
    private static MediaFormat getTrackFormat(int adaptationSetType, Format format, String mediaMimeType, long durationUs) {
        switch (adaptationSetType) {
            case 0:
                return MediaFormat.createVideoFormat(format.f3148id, mediaMimeType, format.bitrate, -1, durationUs, format.width, format.height, null);
            case 1:
                return MediaFormat.createAudioFormat(format.f3148id, mediaMimeType, format.bitrate, -1, durationUs, format.audioChannels, format.audioSamplingRate, null, format.language);
            case 2:
                return MediaFormat.createTextFormat(format.f3148id, mediaMimeType, format.bitrate, durationUs, format.language);
            default:
                return null;
        }
    }
    ...
    private static String getMediaMimeType(Format format) {
        ...
        if ("application/mp4".equals(formatMimeType)) {
    ...
    static boolean mimeTypeIsRawText(String mimeType) {
        return "text/vtt".equals(mimeType) || "application/ttml+xml".equals(mimeType);
    }
    ...
    public Chunk newMediaChunk(PeriodHolder periodHolder, RepresentationHolder representationHolder, DataSource dataSource2, MediaFormat mediaFormat, ExposedTrack enabledTrack2, int segmentNum, int trigger, boolean isMediaFormatFinal) {
        ...
        if (mimeTypeIsRawText(format.mimeType)) {
            return new SingleSampleMediaChunk(dataSource2, dataSpec, 1, format, startTimeUs, endTimeUs, segmentNum, enabledTrack2.trackFormat, null, periodHolder.localIndex);
        }
    ...
    private void processManifest(MediaPresentationDescription manifest) {
        ...
        if (this.periodHolders.size() <= manifest.getPeriodCount()) {
            try {
                ...
                C4016TimeRange newAvailableRange = getAvailableRange(getNowUnixTimeUs());
    ...
    private C4016TimeRange getAvailableRange(long nowUnixTimeUs) {
    ...
    private void notifyAvailableRangeChanged(final C4016TimeRange seekRange) {
