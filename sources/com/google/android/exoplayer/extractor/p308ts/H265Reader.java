...
final class H265Reader extends ElementaryStreamReader {
        ...
        public void readNalUnitData(byte[] data, int offset, int limit) {
            ...
            if (this.lookingForFirstSliceFlag) {
                ...
                if (headerOffset < limit) {
                    if ((data[headerOffset] & ISOFileInfo.DATA_BYTES1) != 0) {
                        z = true;
                    } else {
                        z = false;
                    }
    ...
    private static MediaFormat parseMediaFormat(NalUnitTargetBuffer vps2, NalUnitTargetBuffer sps2, NalUnitTargetBuffer pps2) {
        ...
        if (bitArray.readBit()) {
            ...
            picHeightInLumaSamples -= (bitArray.readUnsignedExpGolombCodedInt() + bitArray.readUnsignedExpGolombCodedInt()) * (chromaFormatIdc == 1 ? 2 : 1);
        }
        bitArray.readUnsignedExpGolombCodedInt();
        bitArray.readUnsignedExpGolombCodedInt();
        ...
        return MediaFormat.createVideoFormat(null, "video/hevc", -1, -1, -1, picWidthInLumaSamples, picHeightInLumaSamples, Collections.singletonList(csd), -1, pixelWidthHeightRatio);
    }
