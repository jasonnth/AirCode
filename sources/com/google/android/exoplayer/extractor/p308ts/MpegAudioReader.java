...
final class MpegAudioReader extends ElementaryStreamReader {
    ...
    private void findHeader(ParsableByteArray source) {
        ...
        for (int i = startOffset; i < endOffset; i++) {
            ...
            if (!this.lastByteWasFF || (data[i] & ISO7816.INS_CREATE_FILE) != 224) {
                found = false;
            } else {
                found = true;
            }
    ...
    private void readHeaderRemainder(ParsableByteArray source) {
        ...
        if (this.frameBytesRead >= 4) {
            ...
            if (!this.hasOutputFormat) {
                this.frameDurationUs = (1000000 * ((long) this.header.samplesPerFrame)) / ((long) this.header.sampleRate);
                this.output.format(MediaFormat.createAudioFormat(null, this.header.mimeType, -1, 4096, -1, this.header.channels, this.header.sampleRate, null, null));
