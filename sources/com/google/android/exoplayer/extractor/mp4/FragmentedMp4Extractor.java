...
public class FragmentedMp4Extractor implements Extractor {
    private static final byte[] PIFF_SAMPLE_ENCRYPTION_BOX_EXTENDED_TYPE = {-94, 57, 79, 82, 90, -101, 79, 20, -94, ISO7816.INS_REHABILITATE_CHV, 108, CVCAFile.CAR_TAG, 124, ISOFileInfo.FMD_BYTE, ISOFileInfo.ENV_TEMP_EF, -12};
    ...
    private void onMoovContainerAtomRead(ContainerAtom moov) {
        ...
        Mapped drmInitData = getDrmInitDataFromAtoms(moov.leafChildren);
        if (drmInitData != null) {
        ...
        if (this.trackBundles.size() == 0) {
            for (int i3 = 0; i3 < trackCount; i3++) {
                this.trackBundles.put(((Track) tracks.valueAt(i3)).f3152id, new TrackBundle(this.extractorOutput.track(i3)));
            }
            this.extractorOutput.endTracks();
        } else {
        ...
        for (int i4 = 0; i4 < trackCount; i4++) {
            Track track2 = (Track) tracks.valueAt(i4);
            ((TrackBundle) this.trackBundles.get(track2.f3152id)).init(track2, (DefaultSampleValues) defaultSampleValuesArray.get(track2.f3152id));
        }
    }

    private void onMoofContainerAtomRead(ContainerAtom moof) throws ParserException {
        parseMoof(moof, this.trackBundles, this.flags, this.extendedTypeScratch);
        Mapped drmInitData = getDrmInitDataFromAtoms(moof.leafChildren);
        if (drmInitData != null) {
            this.extractorOutput.drmInitData(drmInitData);
        }
    }
    ...
    private static TrackBundle parseTfhd(ParsableByteArray tfhd, SparseArray<TrackBundle> trackBundles2, int flags2) {
        ...
        TrackBundle trackBundle = (TrackBundle) trackBundles2.get(trackId);
        if (trackBundle == null) {
            return null;
        }
        if ((atomFlags & 1) != 0) {
    ...
    private static void parseTrun(TrackBundle trackBundle, long decodeTime, int flags2, ParsableByteArray trun) {
        ...
        while (i < sampleCount) {
            ...
            int sampleFlags = (i != 0 || !firstSampleFlagsPresent) ? sampleFlagsPresent ? trun.readInt() : defaultSampleValues.flags : firstSampleFlags;
    ...
    private static ChunkIndex parseSidx(ParsableByteArray atom, long inputPosition) throws ParserException {
        ...
        long timeUs = Util.scaleLargeTimestamp(time, 1000000, timescale);
    ...
    private static Mapped getDrmInitDataFromAtoms(List<LeafAtom> leafChildren) {
        ...
        for (int i = 0; i < leafChildrenSize; i++) {
            ...
            if (child.type == Atom.TYPE_pssh) {
                ...
                if (PsshAtomUtil.parseUuid(psshData) == null) {
                    ...
                    drmInitData.put(PsshAtomUtil.parseUuid(psshData), new SchemeInitData("video/mp4", psshData));
                }
            }
        }
        return drmInitData;
