...
class GifFrameLoader {
    ...
    static class DelayTarget extends SimpleTarget<Bitmap> {
        ...
        public void onResourceReady(Bitmap resource2, GlideAnimation<? super Bitmap> glideAnimation, boolean fromMemoryCache) {
    ...
    public GifFrameLoader(Context context, FrameCallback callback2, GifDecoder gifDecoder2, int width, int height) {
        this(callback2, gifDecoder2, null, getRequestBuilder(context, gifDecoder2, width, height, Glide.get(context).getBitmapPool()));
    }
    ...
    private static GenericRequestBuilder<GifDecoder, GifDecoder, Bitmap, Bitmap> getRequestBuilder(Context context, GifDecoder gifDecoder2, int width, int height, BitmapPool bitmapPool) {
        ...
        return Glide.with(context).using(frameLoader, GifDecoder.class).load(gifDecoder2).mo12478as(Bitmap.class).sourceEncoder(NullEncoder.get()).decoder(frameResourceDecoder).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.NONE).override(width, height);
    }
}
