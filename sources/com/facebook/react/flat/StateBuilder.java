...
final class StateBuilder {
    ...
    /* access modifiers changed from: 0000 */
    public void applyUpdates(FlatShadowNode node) {
    ...
    /* access modifiers changed from: 0000 */
    public void addDrawCommand(AbstractDrawCommand drawCommand) {
        this.mDrawCommands.add(drawCommand);
    }

    /* access modifiers changed from: 0000 */
    public void addAttachDetachListener(AttachDetachListener listener) {
        this.mAttachDetachListeners.add(listener);
    }

    /* access modifiers changed from: 0000 */
    public void enqueueViewManagerCommand(int reactTag, int commandId, ReadableArray commandArgs) {
        this.mViewManagerCommands.add(this.mOperationsQueue.createViewManagerCommand(reactTag, commandId, commandArgs));
    }

    /* access modifiers changed from: 0000 */
    public void enqueueCreateOrUpdateView(FlatShadowNode node, ReactStylesDiffMap styles) {
    ...
    /* access modifiers changed from: 0000 */
    public void ensureBackingViewIsCreated(FlatShadowNode node) {
        if (!node.isBackingViewCreated()) {
    ...
    /* access modifiers changed from: 0000 */
    public void dropView(FlatShadowNode node, int parentReactTag) {
    ...
    private boolean collectStateForMountableNode(FlatShadowNode node, float left, float top, float right, float bottom, float clipLeft, float clipTop, float clipRight, float clipBottom) {
        boolean expectingUpdate = (node.hasNewLayout() || node.isUpdated() || node.hasUnseenUpdates() || node.clipBoundsChanged(clipLeft, clipTop, clipRight, clipBottom)) ? SKIP_UP_TO_DATE_NODES : false;
        if (!expectingUpdate) {
            return false;
        }
        ...
        if (node instanceof AndroidView) {
            ...
            isAndroidView = SKIP_UP_TO_DATE_NODES;
        ...
        if (!isAndroidView && node.isVirtualAnchor()) {
            addNodeRegion(node, left, top, right, bottom, SKIP_UP_TO_DATE_NODES);
        }
        ...
        if (drawCommands != null) {
            shouldUpdateMountState = SKIP_UP_TO_DATE_NODES;
        ...
        if (shouldUpdateMountState) {
            if (node.clipsSubviews()) {
                ...
                this.mOperationsQueue.enqueueUpdateClippingMountState(node.getReactTag(), drawCommands, drawViewIndexMap, commandMaxBottom, commandMinTop, listeners, nodeRegions, regionMaxBottom, regionMinTop, nativeChildren != null ? SKIP_UP_TO_DATE_NODES : false);
            } else {
        ...
        boolean updated = (shouldUpdateMountState || nativeChildren != null || descendantUpdated) ? SKIP_UP_TO_DATE_NODES : false;
    ...
    private void updateNativeChildren(FlatShadowNode node, FlatShadowNode[] oldNativeChildren, FlatShadowNode[] newNativeChildren) {
        ...
        for (FlatShadowNode child3 : newNativeChildren) {
            child3.setNativeParentTag(tag);
        }
    ...
    private boolean processNodeAndCollectState(FlatShadowNode node, float parentLeft, float parentTop, float parentClipLeft, float parentClipTop, float parentClipRight, float parentClipBottom, boolean parentIsAndroidView, boolean needsCustomLayout) {
        ...
        if (!parentIsAndroidView) {
            addNodeRegion(node, left, top, right, bottom, !mountsToView ? SKIP_UP_TO_DATE_NODES : false);
        }
        if (!mountsToView) {
