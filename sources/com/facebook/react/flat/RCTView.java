...
final class RCTView extends FlatShadowNode {
    ...
    public void handleUpdateProperties(ReactStylesDiffMap styles) {
        boolean z = false;
        this.mRemoveClippedSubviews = this.mRemoveClippedSubviews || (styles.hasKey(ReactClippingViewGroupHelper.PROP_REMOVE_CLIPPED_SUBVIEWS) && styles.getBoolean(ReactClippingViewGroupHelper.PROP_REMOVE_CLIPPED_SUBVIEWS, false));
    ...
    public void updateNodeRegion(float left, float top, float right, float bottom, boolean isVirtual) {
        NodeRegion hitSlopNodeRegion;
        if (!getNodeRegion().matches(left, top, right, bottom, isVirtual)) {
            if (this.mHitSlop == null) {
                hitSlopNodeRegion = new NodeRegion(left, top, right, bottom, getReactTag(), isVirtual);
            } else {
                hitSlopNodeRegion = new HitSlopNodeRegion(this.mHitSlop, left, top, right, bottom, getReactTag(), isVirtual);
            }
            setNodeRegion(hitSlopNodeRegion);
        }
    }
