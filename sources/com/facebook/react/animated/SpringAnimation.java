    ...
    private void advance(double realDeltaTime) {
        if (!isAtRest()) {
            ...
            while (this.mTimeAccumulator >= SOLVER_TIMESTEP_SEC) {
                ...
                double bVelocity = tempVelocity2;
                double bAcceleration = ((this.mEndValue - (position + ((SOLVER_TIMESTEP_SEC * aVelocity) * 0.5d))) * tension) - (friction * tempVelocity2);
                double tempVelocity3 = velocity + (SOLVER_TIMESTEP_SEC * bAcceleration * 0.5d);
                double cVelocity = tempVelocity3;
                double cAcceleration = ((this.mEndValue - (position + ((SOLVER_TIMESTEP_SEC * bVelocity) * 0.5d))) * tension) - (friction * tempVelocity3);
