...
public class MessageQueueThreadImpl implements MessageQueueThread {
    ...
    public void runOnQueue(Runnable runnable) {
        if (this.mIsFinished) {
            FLog.m1847w(ReactConstants.TAG, "Tried to enqueue runnable on already finished thread: '" + getName() + "... dropping Runnable.");
        }
    ...
    private static MessageQueueThreadImpl createForMainThread(String name, QueueThreadExceptionHandler exceptionHandler) {
        ...
        if (UiThreadUtil.isOnUiThread()) {
            ...
            UiThreadUtil.runOnUiThread(new Runnable(mqt) {
