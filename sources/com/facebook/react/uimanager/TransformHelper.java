...
public class TransformHelper {
    ...
    public static void processTransform(ReadableArray transforms, double[] result) {
        double[] helperMatrix = (double[]) sHelperMatrix.get();
        ...
        for (int transformIdx = 0; transformIdx < size; transformIdx++) {
            ...
            if ("matrix".equals(transformType)) {
                ...
                MatrixMathHelper.applyTranslate3D(helperMatrix, value.getDouble(0), value.getDouble(1), value.size() > 2 ? value.getDouble(2) : 0.0d);
            } else if ("translateX".equals(transformType)) {
                MatrixMathHelper.applyTranslate2D(helperMatrix, transform.getDouble(transformType), 0.0d);
            } else if ("translateY".equals(transformType)) {
                MatrixMathHelper.applyTranslate2D(helperMatrix, 0.0d, transform.getDouble(transformType));
