...
public class JSTouchDispatcher {
    ...
    public void handleTouchEvent(MotionEvent ev, EventDispatcher eventDispatcher) {
        ...
        if (action == 0) {
            if (this.mTargetTag != -1) {
                FLog.m1807e(ReactConstants.TAG, "Got DOWN touch before receiving UP or CANCEL from last gesture");
            }
            ...
            if (this.mTargetTag == -1) {
                FLog.m1807e(ReactConstants.TAG, "Unexpected state: received touch event but didn't get starting ACTION_DOWN for this gesture before");
            } else if (action == 1) {
                ...
                if (this.mTouchEventCoalescingKeyHelper.hasCoalescingKey(ev.getDownTime())) {
                    ...
                    FLog.m1807e(ReactConstants.TAG, "Received an ACTION_CANCEL touch event for which we have no corresponding ACTION_DOWN");
                }
                this.mTargetTag = -1;
            } else {
                FLog.m1847w(ReactConstants.TAG, "Warning : touch event was ignored. Action=" + action + " Target=" + this.mTargetTag);
            }
        }
    }
    ...
    private void dispatchCancelEvent(MotionEvent androidEvent, EventDispatcher eventDispatcher) {
        boolean z;
        if (this.mTargetTag == -1) {
            FLog.m1847w(ReactConstants.TAG, "Can't cancel already finished gesture. Is a child View trying to start a gesture from an UP/CANCEL event?");
            return;
        }
        if (!this.mChildIsHandlingNativeGesture) {
            z = true;
