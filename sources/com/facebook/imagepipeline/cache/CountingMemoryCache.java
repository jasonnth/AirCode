...
public class CountingMemoryCache<K, V> implements MemoryTrimmable, MemoryCache<K, V> {
    ...
    static class Entry<K, V> {
        ...
        private Entry(K key2, CloseableReference<V> valueRef2, EntryStateObserver<K> observer2) {
        ...
        @VisibleForTesting
        /* renamed from: of */
        static <K, V> Entry<K, V> m1875of(K key2, CloseableReference<V> valueRef2, EntryStateObserver<K> observer2) {
    ...
    public CloseableReference<V> cache(K key, CloseableReference<V> valueRef, EntryStateObserver<K> observer) {
        ...
        synchronized (this) {
            ...
            if (canCacheNewValue(valueRef.get())) {
                Entry<K, V> newEntry = Entry.m1875of(key, valueRef, observer);
    ...
    private synchronized CloseableReference<V> newClientReference(final Entry<K, V> entry) {
        ...
        return CloseableReference.m1872of(entry.valueRef.get(), new ResourceReleaser<V>() {
    ...
    public CloseableReference<V> reuse(K key) {
        ...
        synchronized (this) {
            entry = (Entry) this.mExclusiveEntries.remove(key);
            if (entry != null) {
                Entry<K, V> entry2 = (Entry) this.mCachedEntries.remove(key);
                Preconditions.checkNotNull(entry2);
                Preconditions.checkState(entry2.clientCount == 0);
                clientRef = entry2.valueRef;
                removed = true;
            }
        }
        if (removed) {
