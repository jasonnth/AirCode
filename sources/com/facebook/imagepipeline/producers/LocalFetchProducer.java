...
public abstract class LocalFetchProducer implements Producer<EncodedImage> {
    ...
    protected LocalFetchProducer(Executor executor, PooledByteBufferFactory pooledByteBufferFactory, boolean fileDescriptorEnabled) {
        ...
        this.mDecodeFileDescriptorEnabledForKitKat = fileDescriptorEnabled && VERSION.SDK_INT == 19;
    }
    ...
    public EncodedImage getByteBufferBackedEncodedImage(InputStream inputStream, int length) throws IOException {
        ...
        CloseableReference<PooledByteBuffer> ref2 = null;
        if (length < 0) {
            try {
                ref = CloseableReference.m1871of(this.mPooledByteBufferFactory.newByteBuffer(inputStream));
            } catch (Throwable th) {
                Closeables.closeQuietly(inputStream);
                CloseableReference.closeSafely(ref2);
                throw th;
            }
        } else {
            ref = CloseableReference.m1871of(this.mPooledByteBufferFactory.newByteBuffer(inputStream, length));
        }
