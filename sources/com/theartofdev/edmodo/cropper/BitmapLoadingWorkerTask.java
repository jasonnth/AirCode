...
final class BitmapLoadingWorkerTask extends AsyncTask<Void, Void, Result> {
    ...
    public Result doInBackground(Void... params) {
        try {
            if (!isCancelled()) {
                BitmapSampled decodeResult = BitmapUtils.decodeSampledBitmap(this.mContext, this.mUri, this.mWidth, this.mHeight);
                if (!isCancelled()) {
                    RotateBitmapResult rotateResult = BitmapUtils.rotateBitmapByExif(decodeResult.bitmap, this.mContext, this.mUri);
