...
public class CropOverlayView extends View {
    ...
    public boolean setMultiTouchEnabled(boolean multiTouchEnabled) {
        if (VERSION.SDK_INT < 11 || this.mMultiTouchEnabled == multiTouchEnabled) {
            return false;
        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.mCropWindowHandler.showGuidelines()) {
            if (this.mGuidelines == Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (this.mGuidelines == Guidelines.ON_TOUCH && this.mMoveHandler != null) {
                drawGuidelines(canvas);
            }
        }
        drawBorders(canvas);
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (this.mCropShape != CropShape.RECTANGLE) {
            this.mPath.reset();
            if (VERSION.SDK_INT < 11 || VERSION.SDK_INT > 17 || this.mCropShape != CropShape.OVAL) {
